...
# second stage
f2 <- formula(tip_log ~ shared_trip + shared_trip_authorized +  fare_full |
                trip_start_date + trip_hour + trip_weekday_hour + origin_destination_pairs + pairs_X_start_date)

# naive
f3 <- formula(tip_log ~ shared_trip_authorized + fare_full |
                trip_start_date + trip_hour + trip_weekday_hour + origin_destination_pairs + pairs_X_start_date)
...
for (i in 2:(k-1)){
  ...
  write.csv(collection, "data/estimationResultsAlternativeTipLog.csv", row.names = FALSE)
